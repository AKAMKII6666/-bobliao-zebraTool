		...
		window.zT =
			{
				...
				stringSeleter: function (string, inputNode) {
					...
					if (new RegExp("^[#]").test(string)) {
						string = string.replace("#", "");
						elements = zZ.Zstatic.selector.selectPrototype(
							inputNode,
							"id=" + string
						);
						return elements;
		...
		selectCss: function (nodes, string) {
			...
			for (var i = 0; i < nodes.length; i++) {
				...
				if (nodes[i].childNodes) {
					elements = zZ.fn.joinArr(
						elements,
						zZ.Zstatic.selector.selectCss(nodes[i].childNodes, string)
					);
				}
			}
			return elements;
		...
		selectPrototype: function (nodes, string) {
			...
			for (var i = 0; i < nodes.length; i++) {
				...
				if (nodes[i].childNodes) {
					elements = zZ.fn.joinArr(
						elements,
						zZ.Zstatic.selector.selectPrototype(nodes[i].childNodes, string)
					);
				}
			}
			return elements;
		...
		selectTagName: function (nodes, string) {
			...
			for (var i = 0; i < nodes.length; i++) {
				...
				if (nodes[i].childNodes) {
					elements = zZ.fn.joinArr(
						elements,
						zZ.Zstatic.selector.selectTagName(nodes[i].childNodes, string)
					);
				}
			}
			return elements;
...
zT.insertBefore = function (input) {
	...
	for (var j = 0; j < this.lObj.length; j++) {
		if (typeof input == "string") {
			var adder = document.createElement("div"),
				identity =
					"zTool_appendNode_" + (+new Date() / Math.random()).toString().replace(".", "");
			adder.setAttribute("id", identity);
			document.body.appendChild(adder);
...
zT.css = function (cssName, cssValue) {
	...
	if (!cssValue && cssValue != "") {
		if (speCss) {
			return this.getSpecialCssValue(cssName);
		}
	...
	if (cssValue && cssName) {
		for (var j = 0; j < this.lObj.length; j++) {
			if (cssValue == "remove" && speCss) {
				this.removeSpecialCssValue(cssName, this.lObj[j]);
				continue;
			}
			if (speCss) {
			...
			var changedCss = "",
				//获得行样式
				lineStyle = this.bor("msie")
					? this.lObj[j].style.cssText
					: this.lObj[j].getAttribute("style");
			...
			if (lineStyle) {
				...
				for (var i = 0; i < styleList.length; i++) {
					//除了要设置的属性外 其他的照旧
					if (
						this.trim(styleList[i].toString().split(":")[0]).toLowerCase() !=
							this.trim(cssName).toLowerCase() &&
						styleList[i] != ""
					) {
				...
				if (changedCss == "") {
					this.lObj[j].removeAttribute("style");
				}
				if (this.bor("msie")) {
...
zT.getSpecialCssValue = function (cssName, Obj) {
	if (cssName == "opacity") {
		...
		if (this.lObj[0].style.filter) {
			return (
				parseInt(this.lObj[0].style.filter.replace("alpha(opacity = ", "").replace(")", "")) /
				100
			).toString();
		}
		return 100;
...
zT.setSpecialCssValue = function (cssName, cssValue, Obj) {
	...
	if (cssName == "blur") {
		if (zZ.fn.bor("chrome") || zZ.fn.bor("safari") || zZ.fn.bor("opera")) {
			zTool(Obj)
				.css("-webkit-filter", "blur(" + cssValue + ")")
				.css("-o-filter", "blur(" + cssValue + ")");
		}
	}
};
//删除特殊的CSS值
...
zT.cssNameToJavascript = function (name) {
	if (name.indexOf("-") != "-1") {
		...
		return (
			...
			namearr[1].substring(1, namearr[1].length)
		);
	}
	return name;
...
zT.attr = function (attrName, attrValue) {
	...
	if (attrValue && attrValue) {
		for (var j = 0; j < this.lObj.length; j++) {
			if (
				attrName == "class" &&
				(this.bor("msie 6.0") || this.bor("msie 7.0") || this.bor("msie 8.0"))
			) {
				this.lObj[j].setAttribute("className", attrValue);
			} else if (attrName == "style" && this.bor("msie")) {
...
zT.unbind = function (event, func) {
	...
	for (var i = 0; i < this.lObj.length; i++) {
		...
		if (func && this.lObj[i]["zEvent-club"][event]["arr"].length > 1) {
			...
			//整理一下其中的空元素
			this.lObj[i]["zEvent-club"][event]["arr"] = zZ.fn.clearEmptyItem(
				this.lObj[i]["zEvent-club"][event]["arr"]
			);
...
zT.show = function (p, callback) {
	...
	for (var j = 0; j < this.lObj.length; j++) {
		if (zZ(this.lObj[j]).css("display") == "block") {
			callBackSwitch++;
			continue;
		}
		if (p) {
			...
			zZ(this.lObj[j]).animation(
				...
				function () {
					if (heightcss && !"auto") {
						...
						zZ(this).css("height", "remove");
					}

					if (opacitycss && !"auto") {
						zZ(this).css("opacity", opacitycss);
					} else {
						zZ(this).css("opacity", "remove");
					}

					if (overflowcss && !"auto") {
						zZ(this).css("overflow", overflowcss);
					} else {
						zZ(this).css("overflow", "remove");
					}
...
zT.hide = function (p, callback) {
	...
	var callBackSwitch = 0;
	for (var j = 0; j < this.lObj.length; j++) {
		if (zZ(this.lObj[j]).css("display") == "none") {
			callBackSwitch++;
			continue;
		}
		if (p) {
...
zZ.Zstatic.frameAction = function (ft, as) {
	if (!zZ(as.lpoolObj).attr("_jpool_animation_id")) {
		zZ(as.lpoolObj).attr("_jpool_animation_id", as.lpoolID);
	}
	if (zZ(as.lpoolObj).attr("_jpool_animation_id") == as.lpoolID) {
		...
		for (var i = 0; i < options.length; i++) {
			...
			if (as.options[i][1] == "-") {
				...
				as.options[i][1] = parseFloat(
					zZ(as.lpoolObj).css(as.options[i][0]).toString().replace("px")
				);
			...
			switch (options[i][4]) {
				case "":
				case "--":
					computedNum = zZ.Tween.Linear(
						parseFloat(+new Date()) - startTime,
						startValue,
						totalValue,
						totalTime
					);
					break;
				case "-<":
					computedNum = zZ.Tween.Quint.easeIn(
						parseFloat(+new Date()) - startTime,
						startValue,
						totalValue,
						totalTime
					...
					break;
				case ">-":
					computedNum = zZ.Tween.Quint.easeOut(
						parseFloat(+new Date()) - startTime,
						startValue,
						totalValue,
						totalTime
					...
					break;
				case "<>":
					computedNum = zZ.Tween.Quint.easeInOut(
						parseFloat(+new Date()) - startTime,
						startValue,
						totalValue,
						totalTime
					...
					break;
				case "~~":
					computedNum = zZ.Tween.Bounce.easeOut(
						parseFloat(+new Date()) - startTime,
						startValue,
						totalValue,
						totalTime
			...
			//修复误差
			if (totalValue > 0) {
				if (computedNum > totalValue) {
					computedNum = totalValue;
				}
		...
		for (var i = 0; i < zZ.Zstatic.intervalList.length; i++) {
			if (zZ.Zstatic.intervalList[i]) {
				...
				if (zZ.Zstatic.intervalList[i].state == "play") {
					zZ.Zstatic.intervalList[i].action(
						zZ.Zstatic.frequencyTime,
						zZ.Zstatic.intervalList[i]
					);
				}
				...
				if (zZ.Zstatic.intervalList[i].startTime != "-") {
					...
					if (
						parseInt(zZ.Zstatic.intervalList[i].startTime) +
							parseInt(zZ.Zstatic.intervalList[i].TotalTime) <=
						parseInt(+new Date())
					) {
						zZ(zZ.Zstatic.intervalList[i].lpoolObj).removeAttr(
							"_jpool_animation_id"
						);
						if (zZ.Zstatic.intervalList[i].callBack) {
							zZ.Zstatic.intervalList[i].callBack.apply(
								zZ.Zstatic.intervalList[i].lpoolObj,
								arguments
							);
						}
						delete zZ.Zstatic.intervalList[i];
...
zZ.ajax = function (url, options) {
	...
	var setUpAjax = function () {
		try {
			...
			try {
				//应对微软的XMLHTTPREQUEST4.0 : 2013-08-22
				...
				try {
					...
					try {
						//应对更老的微软XMLHTTPREQUEST
						request = new ActiveXObject("Microsoft.XMLHTTP");
					} catch (failed) {
						//还没有就直接提示用户您浏览器实在太老了，丢不起这脸
						request = false;
					}
				}
			}
		...
		if (!request) {
			alert(
				"Error! Your borwser version is too old, please upgrade your borwser!\n您的浏览器可能版本过低，请升级系统后再继续使用本网页！"
			);
		}
	};
